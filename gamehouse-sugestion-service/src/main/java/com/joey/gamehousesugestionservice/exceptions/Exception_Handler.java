...
import com.joey.gamehousesugestionservice.exceptions.records.ErrorResponse;
...
public class Exception_Handler extends ResponseEntityExceptionHandler {
    ...
    private ResponseEntity<String> suggestionNotFoundExceptionHandler (SuggestionNotFoundException exception) {
        ErrorResponse<String> threatResponse = new ErrorResponse<>(HttpStatus.NOT_FOUND, exception.getMessage());
        LOGGER.error("Error: {}", exception.getMessage());
        return ResponseEntity.status(threatResponse.httpStatus()).body(threatResponse.body());
    }

    @ExceptionHandler(EmptyFieldsExceptions.class)
    private ResponseEntity<String> emptyFieldsExceptionsHandler (EmptyFieldsExceptions exception) {
